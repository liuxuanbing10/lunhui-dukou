...
## 7. 美术去假：AI 资产生产配方（待启动 · 用户 2026-08-04 决定暂不生产）

**根因**：当前观感"假"的不是程序演出，而是**只有程序演出层、缺原画基底**（RainNight 处于"活但假"象限）。锁定的 2.5D 标准本就是"原画基底 + 程序演出"即"真且活"象限——占位阶段美术未产出的预期状态。

**让 AI 生产什么（基底层，按垂直切片优先级）**
- P0（去假最小闭环）：渡口雨夜主背景 plate（远/中/近可拆，用于已有 R3F 视差） + 何叔立绘（含 2–3 表情分层做微表情）。
- P1：汤碗暖光道具透明 PNG（叠/替 R3F sphere） + 其余 7 居民立绘（批量） + 记忆叠影残影（琥珀半透明，memory 相位 ghost）。
- P2：纸人 / 花束 / 钟楼等道具图标。

**工具链**
- 主路径 `ImageGen`（DeferExecuteTool 调用，文生图）：~5–10 credits/张，一次性资产成本可接受；只生 raster，透明/分层靠 prompt 要求或后期抠图。
- 零成本备选 `opencode` + 本地免费图像模型（需用户确认本机是否配；未配则 opencode 只写合成脚本，不负责生图）。
- 后处理 `D:\tools\ImageMagick`（裁剪/转透明/优化/视差合成）——把 AI 图变成游戏资产的必经步，非可选。
- 不用 Blender（2.5D 基底无需 3D；真 3D PoC 属更后期）。

**一致性纪律（"不假"的灵魂）**：prompt 锚定 `theme.ts` 调色板（`rain.base #0b1a2b` / `warm.soul #ffb15c` / `memory.amber #d8a24a`），风格锁"2.5D 插画 / 厚涂或水彩质感 / 雨夜 / 冷暖对比 / 不写实 / 不二次元贴图感"。否则 AI 出图各说各话，反而更假。

**流程**：美术视角出 prompt pack（锚定调色+风格锁定）→ ImageGen 生 P0 → ImageMagick 后处理 → 接入 RainNight/App（背景替程序天空、立绘替占位、汤碗叠暖光）→ 浏览器人工走查"真且活" → 通过再铺 P1 批量。
