 ...
 */
/** 各居民审问台词（id → 台词；元数据从 RESIDENTS 派生，防双源漂移） */
const INTERROGATES: Record<string, NonNullable<LivingTownResident['interrogate']>> = {
  r1: { greeting: '（他没抬头，雨水顺着蓑衣滴落。）你又来了。', prompts: ['你捞过我几次？', '你为什么不回头看钟楼？', '老王……你认识他吗？'] },
  r2: { greeting: '（她笑着把一束花往身后藏了藏。）哟，稀客。', prompts: ['你每晚往渡口放的是什么花？', '你在等谁回来？', '七年前那场船难，你还记得吗？'] },
  r3: { greeting: '（他抬头，眼角全是褶。）吃了吗？', prompts: ['角落那碗面，是给谁的？', '你弟弟……后来怎样了？', '蓑衣人，你看着眼熟？'] },
  r4: { greeting: '（他手指上的朱砂还没干，声音发抖。）师、傅说不能多嘴……', prompts: ['你扎的纸人，夜里会去哪？', '你师傅去哪了？', '你为什么怕我？'] },
  r5: { greeting: '（他调整着齿轮，头也不抬。）又停了。', prompts: ['钟为什么停在 3:17？', '这地方，重复过多少次了？', '你是不是……都记得？'] },
  r6: { greeting: '（他压低嗓门，往船舱瞟了一眼。）那都不是事儿。', prompts: ['七年前那晚，你为什么没出船？', '船舱里那只鞋，是谁的？', '你每晚出船，到底在捞什么？'] },
  r7: { greeting: '（铜哨子挂在胸前，眼神锐利。）熄灯。关门。别问。', prompts: ['你每晚 3:17 去渡口做什么？', '你在等谁？', '你见过我从水里站起来，对吗？'] },
  r8: { greeting: '（他抱着布包，眼睛很亮。）我知道你会来。', prompts: ['你怎么知道我的名字？', '你在等谁？', '郑爷……你认识他吗？'] },
};

/** 活镇居民聚合：由 RESIDENTS（web/src/residents.ts）派生，避免 id/name/role/emoji 双源复制 */
export const livingTownResidents: LivingTownResident[] = Object.values(RESIDENTS).map((meta) => ({
  ...meta,
  interrogate: INTERROGATES[meta.id],
}));
